...
export default function Ninja() {
  ...
  function drawBackground() {
    ...
    var gradient = canvasRef.current
      .getContext("2d")
      .createLinearGradient(0, 0, 0, window.innerHeight);
  ...
  function drawHill(baseHeight, amplitude, stretch, color) {
    canvasRef.current.getContext("2d").beginPath();
    canvasRef.current.getContext("2d").moveTo(0, window.innerHeight);
    canvasRef.current
      .getContext("2d")
  ...
  return (
    <div className="w-screen h-full bg-[#02121d]">
      <div className="container mx-auto cursor-pointer flex flex-col justify-center items-center h-full font-body">
        <div
          ref={scoreRef}
          className="absolute top-[10rem] right-1/2 -translate-x-1/2 text-2xl font-bold"
        ></div>
        <canvas ref={canvasRef} width="375" height="375" />
        <div
          ref={introRef}
          className={`w-[200px] h-[150px] absolute font-semibold text-lg text-center`}
        >
        ...
        <button
          onClick={restartGame}
          ref={restartRef}
          className={`absolute border-none w-[120px] h-[120px] hidden rounded-full text-white bg-red-400 font-semibold text-xl cursor-pointer`}
        >
          RESTART
        </button>
      </div>
