...
import { getScoreBoardContract } from "../../contracts/ScoreBoardHelper";

export default function Ninja() {
  async function createPlayer() {
    const { scoreBoardWriteContract, scoreBoardReadContract } =
      await getScoreBoardContract();
    let tx = await scoreBoardWriteContract.create(oneTimeScore);
    await tx.wait();
    let [score, address] = await scoreBoardReadContract.get(0);

    console.log(score.toNumber(), address);
    console.log(scoreBoardReadContract.getAllPlayers().toNumber());
    console.log(scoreBoardReadContract.getTopPlayers(2).toNumber());
  }

  async function addScoreToPlayer() {
    const { scoreBoardWriteContract, scoreBoardReadContract } =
      await getScoreBoardContract();

    let _tx = await scoreBoardWriteContract.addScore(1, oneTimeScore);
    await _tx.wait();
    let [score, address] = await scoreBoardReadContract.get(1);
    console.log(score.toNumber(), address);
  }
  ...
  return (
    <div className="w-full h-screen bg-[#02121d] lg:pt-20">
        ...
        <div className="w-full rounded bg-[#0a1f2f] p-10">
          <div className="flex items-start justify-between">
            ...
            <div className="flex-1 flex flex-col items-center space-y-2">
              ...
              <button className="bg-[#28dbd1] text-[#0a1f2f] text-lg hover:text-[#28dbd1] hover:border-[#28dbd1] hover:skew-x-0 duration-300 border border-transparent hover:bg-[#0a1f2f] font-semibold h-10 w-32 rounded flex justify-center items-center">
                Mint
              </button>
            </div>
